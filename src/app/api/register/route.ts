...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!phoneRaw)
      return NextResponse.json({ ok: false, message: "شماره موبایل را وارد کنید" }, { status: 400 });
    if (!isPhone(phoneRaw))
    ...
    if (byEmail && !email.endsWith("@liceno.ir"))
      return NextResponse.json({ ok: false, message: "این ایمیل قبلاً ثبت شده است" }, { status: 400 });

    const generatedPassword = password || randomBytes(24).toString("hex");
